...
void process_setup(pid_t pid, const char* program_name) {
    ...
    for (auto seg = pgm.begin(); seg != pgm.end(); ++seg) {
        void *phy_ptr = memory_virtual_to_physical(ptable[pid].pagetable, (uintptr_t)seg.va());
        memset(phy_ptr, 0, seg.size());
        memcpy(phy_ptr, seg.data(), seg.data_size());
    }
    ...
    memory_map(ptable[pid].pagetable, stack_addr, (uintptr_t)stack_ptr, PTE_PWU);
